...
namespace Noo.Tools.Triangulation
{
	public static class MathUtils
    {
		...
		public static bool IsPointInsideCircumcircle(Vector2 p0, Vector2 p1, Vector2 p2, Vector2 pointToCheck)
		{
			...
			(
				new float4(p0.x, p0.y, math.dot(p0, p0), 1f),
				new float4(p1.x, p1.y, math.dot(p1, p1), 1f),
				new float4(p2.x, p2.y, math.dot(p2, p2), 1f),
				new float4(pointToCheck.x, pointToCheck.y, math.dot(pointToCheck, pointToCheck), 1f)
			);

			return math.determinant(matrix) >= 0f;
		}
