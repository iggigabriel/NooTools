...
{
    public abstract class NuiPropertyDrawerFloatVectors<T> : NuiPropertyDrawer<T>
    {
        protected readonly Dictionary<string, float> fieldValues = new();
        ...
        struct InnerField
        {
            ...
            public FieldMouseDragger<float> labelDragController;
            public FocusForwardManipulator labelFocusController;
        }

        readonly Dictionary<string, InnerField> innerFields = new();

        protected NuiPropertyDrawerFloatVectors(params string[] labels)
        {
        ...
        protected override void OnPropertyCreate()
        {
            ...
            foreach (var (fieldName, _) in fieldValues)
            {
                var field = new InnerField()
                {
                    ...
                    input = NuiPool.Rent<FloatField>().WithClass("dtk-input-field"),
                ...
                field.labelDragController = new FieldMouseDragger<float>(field.input);
        ...
        private void OnChanged(ChangeEvent<float> e)
        {
            foreach (var (fieldName, field) in innerFields)
            {
